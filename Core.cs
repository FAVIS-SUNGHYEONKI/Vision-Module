...
namespace Vision
{
    ...
    ///
    /// 각 스텝은 이 객체를 통해 이미지를 받고, 처리 결과를 다음 스텝으로 전달한다.
    /// CogImage / MatImage 중 하나만 설정해도 각 스텝 기반 클래스(CogStepBase / CvStepBase)가
    /// 실행 시점에 필요한 포맷으로 자동 변환한다.
    ///
    /// 사용 패턴:
    ...
    public class VisionContext : IDisposable
    {
        ...
        /// <summary>
        /// 파이프라인 전체 성공 여부.
        /// 어느 스텝이라도 SetError()를 호출하면 false로 전환된다.
        /// ContinueOnFailure = true 인 스텝은 false 상태에서도 다음 스텝이 실행된다.
        /// </summary>
        public bool IsSuccess { get; set; } = true;

        /// <summary>
        /// 스텝별 오류 메시지 누적 목록.
        /// SetError() 호출 시 메시지가 추가된다.
        /// </summary>
        ...
        ///   "VisionPro.Blob.0", "VisionPro.Blob.1", ...       → CogBlobResults
        ///
        /// 동일 타입의 스텝이 여러 번 실행되면 인덱스 번호가 증가한다.
        /// </summary>
        public Dictionary<string, object> Data { get; } = new Dictionary<string, object>();

        /// <summary>
        /// 스텝 실행 실패를 기록한다.
        /// IsSuccess를 false로 설정하고 오류 메시지를 Errors에 추가한다.
        /// </summary>
        ...
        /// <summary>
        /// 보유 중인 이미지 리소스(MatImage, CogImage)를 해제한다.
        /// </summary>
        public void Dispose()
        {
            MatImage?.Dispose();
    ...
    {
        /// <summary>
        /// 스텝의 고유 이름. PipelineManager가 XML 저장/로드 시 type 속성으로 사용한다.
        /// 예: "VisionPro.Caliper", "VisionPro.Blob", "OpenCV.Threshold"
        /// </summary>
        string Name { get; }

        /// <summary>
        /// 사용자가 설정한 표시 이름. 설정하지 않으면 Name을 반환한다.
        /// 파이프라인 목록, 결과 탭 등 UI 표시에 사용된다.
        /// </summary>
        string DisplayName { get; set; }

        /// <summary>
        /// true이면 이 스텝이 실패(context.IsSuccess = false)해도 다음 스텝을 계속 실행한다.
        /// false이면 실패 즉시 파이프라인이 중단된다.
        /// </summary>
        bool ContinueOnFailure { get; }

        /// <summary>
        /// 스텝의 처리를 실행한다. 결과는 context에 기록된다.
        /// 오류 발생 시 context.SetError()를 호출해야 한다(예외 throw 지양).
        /// </summary>
    ...
    /// LoadAll()이 LoadParams()를 호출한다.
    /// XElement el은 해당 요소이며, 하위 요소를 자유롭게 추가할 수 있다.
    /// </summary>
    public interface IStepSerializable
    {
        /// <summary>스텝 파라미터를 XML 요소에 기록한다.</summary>
        /// <param name="el">쓰기 대상 XElement</param>
        void SaveParams(XElement el);

        /// <summary>XML 요소에서 스텝 파라미터를 복원한다.</summary>
    ...
    ///
    /// GUI는 이 인터페이스를 통해 스텝마다 독립적인 Region을 설정/표시할 수 있다.
    /// Region = null 이고 RegionRequired = false 이면 전체 이미지를 대상으로 실행한다.
    /// </summary>
    public interface IRegionStep
    {
        /// <summary>검사 영역(ROI). null이면 전체 이미지가 대상이다.</summary>
        CogRectangleAffine Region { get; set; }

        /// <summary>
        /// Region이 반드시 필요하면 true, 없어도 동작 가능하면 false.
        /// VisionForm은 true인 스텝에 Region이 없으면 실행 전 경고를 표시한다.
        /// </summary>
    ...
    /// 선택된 스텝 이전 위치에서는 실행되지 않는다.
    /// (이미지 변환 스텝만 선행 실행되어 정확한 단일 스텝 결과를 확인할 수 있다.)
    ///
    ...
    /// 인접 스텝 간 타입 불일치를 감지하여 시각적 경고를 표시한다.
    /// 이 인터페이스를 구현하지 않는 스텝은 Any→Any로 취급한다.
    /// </summary>
    ...
    {
        /// <summary>제약 없음 — 어떤 이미지도 허용/출력한다.</summary>
        Any,
