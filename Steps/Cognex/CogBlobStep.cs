...
namespace Vision.Steps.VisionPro
{
    ...
    ///
    /// 동작:
    ///   - 그레이스케일 이미지를 임계값으로 이진화하여 Blob을 검출한다.
    ///   - Region (CogRectangleAffine)을 검색 영역으로 사용한다.
    ///   - 결과는 VisionContext.Data["VisionPro.Blob.{N}"] 키에 저장된다.
    ///     같은 파이프라인에 Blob 스텝이 여러 개 있으면 N이 자동으로 증가한다.
    ///
    /// 결과 타입: CogBlobResultCollection
    ///   각 CogBlobResult는 Area, Centroid, GetBoundary() 등을 제공한다.
    ///   GetBoundary()로 CogPolygon(외곽선)을 얻어 Display에 직접 추가할 수 있다.
    ///
    ...
    ///   SegmentationParams(Mode, Polarity, HardFixedThreshold, SoftFixedThresholdLow/High),
    ...
    public class CogBlobStep : CogStepBase, IStepSerializable, IRegionStep, IInspectionStep
    {
        ...
        /// <summary>검색 영역. null이면 전체 이미지를 검색한다.</summary>
        ...
        /// <summary>Blob은 Region 없이도 실행 가능하다 (전체 이미지 검색).</summary>
        public bool RegionRequired => false;

        /// <summary>
        /// Blob 도구를 실행하고 결과를 컨텍스트에 기록한다.
        ///
        ...
        /// <summary>
        /// Region, SegmentationParams, ConnectivityMinPixels를 XML에 저장한다.
        ...
        /// <summary>
        /// XML 요소에서 Region, SegmentationParams, ConnectivityMinPixels를 복원한다.
        /// 누락된 요소는 기본값으로 대체된다.
        /// </summary>
